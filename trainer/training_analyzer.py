...
class FailReason:
    MASK_ZERO       = "Mask=0 (No Resource)"
    PATH_BLOCKED    = "Path Blocked"
    TIMEOUT         = "Timeout"
    VNF_FAIL        = "VNF Deploy Fail"
    NO_PROGRESS     = "No Progress"
    BW_EXHAUSTED    = "BW Exhausted"
    TRAPPED         = "Trapped"
    UNKNOWN         = "Unknown"
...
class TrainingAnalyzer:
    ...
    def report(self):
        ...
        if not self.records:
            logger.warning(" TrainingAnalyzer: 没有记录，跳过报告生成")
            return
    ...
    def _plot_epsilon_chart(self):
        ...
        if not eps_recs:
            logger.warning(" 没有 epsilon 数据，跳过 epsilon 图表（检查 agent 是否正确传入）")
    ...
    def _write_text_report(self):
        ...
        if util_candidates:
            ...
            w(f"    瓶颈资源: {bottleneck[0]} (占用率最高 {bottleneck[1]*100:.1f}%)")
        else:
