...
补丁说明（相对原版）：
  [P4] _get_local_embedding 新增 req=None 参数，优先用传入的历史 req 构建 req_vec，
       不再每次都读 self.env.current_request，修复高层训练时 node_embs / graph_emb
       req 语义错位的问题。
  [P5] _get_local_embedding 的 except 块改为 logger.warning，不再静默吞异常。
  [P6] _extract_state_embedding 的异常 fallback 统一返回 hidden_dim 维零向量，
       与 _get_graph_embedding 的 fallback 口径对齐。
"""
...
class HRLAgentAction:
    ...
    def select_action(
        ...
        try:
            ...
            if getattr(self, '_single_dqn', False):
                ...
                self.current_goal_emb = torch.zeros(
                    1, self.goal_dim, device=self.device)
    ...
    def _get_high_candidate_info(self, state, unconnected_dests, action_mask=None):
        """统一候选信息查询: 高层目标选择用"""
        cand = list(unconnected_dests or [])
        if action_mask is not None:
            cand = [i for i in cand if 0 <= i < len(action_mask) and action_mask[i] > 0]
        ctl = getattr(self.env, 'high_level_controller', None) if self.env is not None else None
        if ctl is not None and hasattr(ctl, 'get_high_level_candidates'):
            try:
                info = ctl.get_high_level_candidates()
                if info is not None and len(info.get('indices', [])) > 0:
                    return {
                        'indices': info.get('indices', []),
                        'node_embs': self._get_local_embedding(state),
    ...
    def _get_low_candidate_info(self, state, action_mask=None):
        """统一候选信息查询: 低层动作选择用"""
        real_state = state[0] if isinstance(state, tuple) else state
        cur = 0
        if self.env is not None and hasattr(self.env, 'current_node_location'):
            cur = int(self.env.current_node_location)
        elif hasattr(real_state, 'current_position'):
            cur = int(real_state.current_position)
        elif isinstance(real_state, dict) and 'current_position' in real_state:
            cur = int(real_state['current_position'])
        ctl = getattr(self.env, 'low_level_controller', None) if self.env is not None else None
        if ctl is not None and hasattr(ctl, 'get_low_level_candidates'):
            try:
                info = ctl.get_low_level_candidates()
                if info is not None and len(info.get('indices', [])) > 0:
                    return {
                        'indices': info.get('indices', []),
                        'current_node': info.get('current_node', cur),
                        'features': info.get('features', None),
                    }
            except Exception:
                pass
        if action_mask is not None:
            idx = np.where(np.asarray(action_mask) > 0)[0].tolist()
            return {'indices': idx, 'current_node': cur, 'features': None}
        return {'indices': [], 'current_node': cur, 'features': None}

    def _select_subgoal(self, state, unconnected_dests, action_mask=None) -> int:
        """高层目标选择 — 统一走 high_policy.select_goal() + candidate 路径"""
        graph_emb = self._get_graph_embedding(state)
        cand = self._get_high_candidate_info(state, unconnected_dests, action_mask)
        cand_idx = cand.get('indices') or []
        cand_feats = self._candidate_feats_from_info(cand)
        mask_t = None
        if action_mask is not None:
            mask_t = torch.as_tensor(action_mask, dtype=torch.float32, device=self.device)
        with torch.no_grad():
            try:
                goal_idx_t, goal_emb = self.high_policy.select_goal(
                    state_emb=graph_emb,
                    valid_goals_mask=mask_t,
                    epsilon=self.epsilon_high,
                    candidate_indices=cand_idx,
                    candidate_node_embs=cand.get('node_embs'),
                    candidate_local_feats=cand_feats,
                )
                subgoal = int(goal_idx_t.item())
                self.current_goal_emb = goal_emb
            except (TypeError, AttributeError):
                
                q_values, goal_emb, _ = self.high_policy(graph_emb, return_subgoal=True)
                if mask_t is not None:
                    mt = mask_t if mask_t.dim() == 2 else mask_t.unsqueeze(0)
                    q_values = torch.where(mt > 0, q_values,
                                           torch.tensor(-1e9, device=self.device))
                valid_idx = torch.nonzero(
                    (mask_t > 0) if mask_t is not None else torch.ones_like(q_values.squeeze())
                ).flatten()
                if random.random() < self.epsilon_high and len(valid_idx) > 0:
                    subgoal = int(valid_idx[torch.randint(0, len(valid_idx), (1,))].item())
                else:
                    subgoal = int(q_values.argmax(dim=1).item())
                self.current_goal_emb = goal_emb
        self.current_subgoal = subgoal
        self._last_high_action_meta = {
            'used_candidate_path':   len(cand_idx) > 0,
            'candidate_indices':     cand_idx,
            'candidate_local_feats': cand_feats,
            'local_goal_idx':        cand_idx.index(subgoal) if subgoal in cand_idx else None,
            'action_mask':           None if action_mask is None else np.asarray(action_mask).copy(),
        }
        return subgoal

    def _candidate_feats_from_info(self, cand):
        if cand is None:
            return None
        feats = cand.get('features')
        if feats is not None and self.env is not None and getattr(self.env, '_ablation_candidate_feats', False):
            ...
            with torch.no_grad():
                if self.encoder is not None:
                    ...
                    node_embeddings = self.encoder(
                        _x, _ei, _ea, batch=_b,
                        tree_edge_index=_tei, dest_mask=_dest, req_vec=_req
                    )
                else:
    ...
    def _select_low_action(self, state, action_mask: Optional[np.ndarray]) -> int:
        """低层动作选择 — 统一走 low_policy.select_action() + candidate 路径"""
        graph_emb = self._get_local_embedding(state)
        if self.current_goal_emb is None:
            self._generate_goal_embedding(state)
        mask_tensor = (
            torch.as_tensor(action_mask, dtype=torch.float32, device=self.device).unsqueeze(0)
            if action_mask is not None else None
        )
        cand = self._get_low_candidate_info(state, action_mask)
        cand_feats = self._candidate_feats_from_info(cand)
        with torch.no_grad():
            try:
                action_t, _ = self.low_policy.select_action(
                    state_emb=graph_emb,
                    goal_emb=self.current_goal_emb,
                    action_mask=mask_tensor,
                    epsilon=self.epsilon_low,
                    candidate_indices=cand.get('indices'),
                    current_node_idx=cand.get('current_node'),
                    candidate_local_feats=cand_feats,
                )
            except TypeError:
                
                action_t, _ = self.low_policy.select_action(
                    graph_emb, self.current_goal_emb, mask_tensor,
                    epsilon=self.epsilon_low
                )
        action = int(action_t.item())
        cand_idx = cand.get('indices') or []
        self._last_low_action_meta = {
            'used_candidate_path': len(cand_idx) > 0,
            'local_action_idx': cand_idx.index(action) if action in cand_idx else None,
            'candidate_indices': cand_idx,
            'candidate_local_feats': cand_feats,
            'current_node_idx': cand.get('current_node'),
            'action_mask': None if action_mask is None else np.asarray(action_mask).copy(),
    ...
    def _get_local_embedding(self, state, req=None):
        """[TA-HGRL Fix] 返回完整节点序列 [1, N, H]，保留N维供Attention审视

        Args:
            state: PyG Data 对象或 tuple
            req:   可选，历史请求字典（高层训练时传入），传入时优先使用，
                   避免用当前 env.current_request 污染历史样本的 embedding。
        """
        ...
        try:
            ...
            logger.warning(
                f"[_get_local_embedding] encoder 调用失败，降级为 graph_embedding: {_e}"
            )
            return self._get_graph_embedding(state)
    ...
    def _get_graph_embedding(self, state, req=None):
        ...
        try:
            ...
            if hasattr(real_state, 'batch') and real_state.batch is not None:
                ...
                out = self.encoder(real_state.x.to(self.device), _ei, _ea,
                                   batch=real_state.batch.to(self.device), tree_edge_index=_tei,
