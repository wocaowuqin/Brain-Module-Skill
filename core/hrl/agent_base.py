...
class HRLAgentBase:
    ...
    def __init__(self, config, encoder=None, phase=3, goal_strategy='adaptive', env=None, **kwargs):
        ...
        self.n_goals = kwargs.get('high_action_dim', env_cfg.get('nb_high_level_goals', 28))
        ...
        self._single_dqn = (_ablation_variant == 'single_dqn')
        if self._single_dqn:
            logger.info(" [Ablation] 单层DDQN模式：高层策略禁用，goal_emb全零")
            _ablation_variant = 'full'

        
        
        self._ablation_variant = 'single_dqn' if self._single_dqn else _ablation_variant
        _req_dim = config.get('gnn', {}).get('req_dim', 3)
        _node_dim = config.get('gnn', {}).get('node_feat_dim', 28)
        _coupling_mode = config.get('gnn', {}).get('coupling_mode', 'full')
        _use_aux_head  = config.get('gnn', {}).get('use_aux_head', True)
        if _ablation_variant == 'full':
            ...
            self.encoder = TreeTransformerEncoder(
                ...
                num_heads=int(config.get('gnn', {}).get('num_heads', 4)),
                req_dim=_req_dim,
                coupling_mode=_coupling_mode,
                use_aux_head=_use_aux_head,
            ).to(self.device)
        elif _ablation_variant == 'gat':
            from core.gnn.gat_encoder import GATEncoder
            self.encoder = GATEncoder(
                node_dim=_node_dim,
                hidden_dim=self.hidden_dim,
                num_heads=config.get('gnn', {}).get('num_heads', 4),
                req_dim=_req_dim,
        ...
        _enc_name = self.encoder.__class__.__name__
        _has_req = getattr(self.encoder, 'req_fc', None) is not None
        _has_aux = getattr(self.encoder, 'use_aux_head', False)
        _coupling = getattr(self.encoder, 'coupling_mode', 'none')
        logger.info(
            f'Encoder mounted: variant={_ablation_variant}, class={_enc_name}, '
            f'req_fusion={_has_req}, aux_head={_has_aux}, coupling={_coupling}'
        )
        ...
        # [Fix-B] candidate_feat_dim: 8 (6 original + incoming BW ratio + directed reuse flag)
        ...
        low_config = {
            ...
            'dropout': config.get('dropout', 0.1),
            'candidate_feat_dim': config.get('candidate_feat_dim', 8),
        }
        ...
        self._last_high_action_meta = None
        self._last_low_action_meta  = None
        self._last_next_low_action_meta = None
    ...
    def train(self):
        ...
        if self.encoder is not None:
            self.encoder.train()
        if getattr(self, 'goal_embedding', None) is not None:
            self.goal_embedding.train()
    ...
    def save(self, path: str):
        torch.save({
            ...
            'target_low_policy':  self.target_low_policy.state_dict(),
            'encoder':            self.encoder.state_dict(),
            'target_encoder':     self.target_encoder.state_dict()
                                  if getattr(self, 'target_encoder', None) is not None else None,
    ...
    def load(self, path: str):
        ...
        if 'target_high_policy' in ckpt:
            self.target_high_policy.load_state_dict(ckpt['target_high_policy'])
        else:
            self.target_high_policy.load_state_dict(self.high_policy.state_dict())

        if 'low_policy' in ckpt:
            self.low_policy.load_state_dict(ckpt['low_policy'])
        if 'target_low_policy' in ckpt:
            self.target_low_policy.load_state_dict(ckpt['target_low_policy'])
        else:
            self.target_low_policy.load_state_dict(self.low_policy.state_dict())
        ...
        if 'encoder' in ckpt:
            ...
            _miss, _unexp = self.encoder.load_state_dict(ckpt['encoder'], strict=False)
            if _miss or _unexp:
                logger.warning(f"[Encoder load] strict=False missing={list(_miss)[:6]} unexpected={list(_unexp)[:6]}")

        if getattr(self, 'target_encoder', None) is not None:
            if ckpt.get('target_encoder') is not None:
    ...
    def register_encoder_to_optimizer(self, lr=None):
        """兼容保留：检查encoder是否已在optimizer_low中，避免重复注册"""
        if self.encoder is None:
            return False
        enc_ids = {id(p) for p in self.encoder.parameters()}
        for pg in self.optimizer_low.param_groups:
            if any(id(p) in enc_ids for p in pg['params']):
                logger.info("ℹ encoder 已在 optimizer_low 中，跳过重复注册")
                return False
