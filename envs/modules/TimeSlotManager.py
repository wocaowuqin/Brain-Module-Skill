...
class TimeSlotManager:
    ...
    def __init__(self, env, config: dict):
        ...
        self.slot_queue: List[dict] = []
        self.current_slot_index: int = 0
        self.max_slot_index: int = 0
    ...
    def reset(self) -> None:
        ...
        每个 Episode 开始时调用。
         ...
         时钟（time_step）不归零——时间在整个仿真生命周期内单调递增。
           归零会导致 lifecycle 里 expire_time 高的请求永远无法到期释放，
           资源持续积累无法回收。
         请求指针也不重置——跨 Episode 持续线性推进，到头后由
    ...
    def _trigger_expiry_check(self) -> None:
        ...
        try:
            ...
            if self._expiry_log_counter % 10 == 0 and active_count > 0:
                nearest = min(info['expire_time'] for info in req_mgr.active_requests.values())
                furthest = max(info['expire_time'] for info in req_mgr.active_requests.values())

                
                remaining_times = [
                    info['expire_time'] - self.time_step
                    for info in req_mgr.active_requests.values()
                ]
                avg_remaining = sum(remaining_times) / len(remaining_times)

                logger.info(
                    f"[DEBUG_ACTIVE] t={self.time_step:.3f} | "
                    f"活跃={active_count} | "
                    f"最近过期={nearest:.3f}(还剩{nearest - self.time_step:.2f}s) | "
                    f"最远过期={furthest:.3f}(还剩{furthest - self.time_step:.2f}s) | "
                    f"平均剩余={avg_remaining:.2f}s | "
                    f"时间偏移={getattr(self, '_time_offset', 0.0):.1f}s"
                )
